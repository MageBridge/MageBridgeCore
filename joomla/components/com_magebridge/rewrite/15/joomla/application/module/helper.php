...
define('MAGEBRIDGE_MODULEHELPER_OVERRIDE', 1);
...
class JModuleHelper
{
	...
	function &getModule($name, $title = null )
	{
        ...
        if(is_object($result) && !empty($result->position) && MageBridgeTemplateHelper::allowPosition($result->position) == false) $result = null;
	...
	function renderModule($module, $attribs = array())
	{
        ...
        if(is_object($module) && !empty($module->position) && MageBridgeTemplateHelper::allowPosition($module->position) == false) return null;
		...
		// Load the module
        if (!isset($module->user)) $module->user = 0;
