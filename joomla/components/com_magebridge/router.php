...
function MagebridgeBuildRoute(&$query)
{
    ...
    if (in_array($query['view'], ['product', 'category'])) {
        $query['view'] = 'root';
    }
    ...
    } elseif (!empty($query['request']) && (empty($root_item_id) && $query['view'] == 'root')) {
        $segments[] = $query['request'];
    }
    ...
    $unset_elements = ['view', 'task', 'request', 'layout', 'format', 'SID', 'language', 'id'];
...
function MagebridgeParseRoute($segments)
{
    ...
    if (MageBridgeUrlHelper::enforceRootMenu()) {
        ...
        $current_path = JUri::getInstance()->toString(['path']);
