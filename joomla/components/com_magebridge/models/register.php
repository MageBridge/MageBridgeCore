...
class MageBridgeModelRegister 
{
	 ...
	 */
	const MAGEBRIDGE_SEGMENT_STATUS_SYNCED = 1;
	...
	public function add($type = null, $name = null, $arguments = null)
	{
		...
		if (in_array($type, array('filter', 'block', 'api', 'widget'))) {
	...
	public function getDataById($id = null)
	{
		...
		if (isset($segment['data'])) return $segment['data'];
	}
	...
	{
		if (!empty($id)) {
			MageBridgeModelDebug::getInstance()->warning('Use of fourth argument in method MageBridgeModelRegister::get() is deprecated');
		}
		...
		if (!empty($this->data)) {
			foreach ($this->data as $index => $segment) {
				...
				} else if (empty($id) && isset($segment['type']) && $segment['type'] == $type && isset($segment['name']) && $segment['name'] == $name) {
					$match = true;

				} else if (empty($id) && empty($name) && isset($segment['type']) && $segment['type'] == $type) {
					$match = true;
				}
	...
	{
		if (!empty($id)) {
			MageBridgeModelDebug::getInstance()->warning('Use of fourth argument in method MageBridgeModelRegister::getData() is deprecated');
		}
	...
	{
		MageBridgeModelDebug::getInstance()->warning('Method MageBridgeModelRegister::getSegment() is deprecated');
	...
	{
		MageBridgeModelDebug::getInstance()->warning('Method MageBridgeModelRegister::getSegmentData() is deprecated');
	...
	{
		MageBridgeModelDebug::getInstance()->warning('Method MageBridgeModelRegister::getSegmentById() is deprecated');
	...
	public function getPendingRegister()
	{
		...
		if (!empty($data) && empty($data['meta'])) {
			$data['meta'] = array(
				'type' => 'meta',
				'name' => null,
				'arguments' => MageBridgeModelBridgeMeta::getInstance()->getRequestData(),
			);
		}
		...
		if (!empty($data)) {
			foreach ($data as $id => $segment) {
				...
				if (!empty($segment['data']) && !empty($segment['type'])) {
					...
					switch($segment['type']) {
						case 'block':
							if (isset($segment['meta']['allow_caching']) && $segment['meta']['allow_caching'] == 1 &&
								isset($segment['meta']['cache_lifetime']) && $segment['meta']['cache_lifetime'] > 0) {
					...
					if (!empty($cache)) $cache->store($segment['data']);
				}
			}
		}
	}
