...
class MageBridgeModelBridge 
{
	...
	public function build()
	{
		...
		if (count($register->getPendingRegister()) > 0) {
			...
			foreach ($register->getPendingRegister() as $segment) {
				switch($segment['type']) {
					...
					case 'block':
						...
						break;
					case 'widget':
						MageBridgeModelDebug::getInstance()->notice('Pending Segment widget: '.$segment['name']);
						break;
					default:
			...
			} else if ($application->isAdmin() && (JFactory::getApplication()->input->getCmd('option') != 'com_magebridge' || JFactory::getApplication()->input->getCmd('view') != 'root')) {
				$proxy->setAllowRedirects(false);
			}
	...
	{
		$session_value = JFactory::getApplication()->input->getCmd('frontend', null, 'cookie');
		if(empty($session_value)) $session_value = JFactory::getSession()->get('magebridge.cookie.frontend');
	...
	public function setMageSession($mage_session = null)
	{
		...
		JFactory::getSession()->set('magebridge.cookie.frontend', $mage_session);
		return JFactory::getApplication()->input->getCmd('frontend', null, 'cookie');
	}
	 ...
	 * @return bool
	 */
	static public function mijosef()
	{
	...
	{
		if (in_array(JFactory::getApplication()->input->getCmd('format'), array('xml', 'json', 'ajax'))) {
			return true;
		}
		...
		} else if (JFactory::getApplication()->input->getCmd('view') == 'ajax') {
			$check_xrequestedwith = false;
		}
