...
class MageBridgeModelUser
{
	...
	public function create($user, $empty_password = false)
	{
		...
		if (empty($result))
		{
			...
			$this->app->triggerEvent('onUserBeforeSave', array($data, true, $data));
			...
			$this->app->triggerEvent('onUserAfterSave', array($data, true, true, null));
	...
	public function synchronize($user)
	{
		...
		if (empty($user['password_clear']))
		{
			...
			$jform  = $this->app->input->get('jform', array(), 'post');
