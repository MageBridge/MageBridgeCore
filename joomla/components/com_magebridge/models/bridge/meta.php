...
class MageBridgeModelBridgeMeta extends MageBridgeModelBridgeSegment
{
	...
	{
		return MageBridgeModelRegister::getInstance()->getData('meta');
	}
	...
	public function getRequestData()
	{
		...
		if (empty($this->_meta_data) || !is_array($this->_meta_data)) {
			...
			$app_type = MageBridgeStoreHelper::getInstance()->getAppType();
			$app_value = MageBridgeStoreHelper::getInstance()->getAppValue();
			...
			$arguments = array(
				...
				'joomla_conf_caching' => $application->getCfg('caching',60),
				'joomla_conf_lifetime' => ($application->getCfg('lifetime',60)*60),
				...
				'request_id' => md5(JURI::current().serialize(JFactory::getApplication()->input->get('get'))),
				'post' => (!empty($_POST)) ? $_POST : null,
				'http_referer' => $bridge->getHttpReferer(),
				//'http_referer' => $session->get('magebridge.http_referer'),
