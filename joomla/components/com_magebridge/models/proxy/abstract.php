...
class MageBridgeModelProxy
{
	...
	public function isValidResponse($data = null)
	{
		...
		if (empty($data['meta']))
		{
			...
			{
				if (JRequest::getCmd('option') == 'com_magebridge' && JRequest::getCmd('view') == 'root')
				{
					return false;
				}
			}
	...
	public function getCURL($url, $type = 'get', $arguments = null, $run_bridge = false)
	{
		...
		if (empty($default_sessname))
		{
			$default_sessname = 'PHPSESSID';
		}
		...
		if (!empty($matches))
		{
			foreach ($matches[0] as $index => $match)
			{
				...
				{
					if ($sessionName == $default_sessname)
					{
						$session->set('magento_session', $sessionId);
					...
					{
						if ($sessionName == 'persistent_shopping_cart')
						{
