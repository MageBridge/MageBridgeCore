...
class plgMagentoMageBridge extends JPlugin
{
     ...
     * @return bool
     */
    public function mageCustomerDeleteAfter($arguments = array())
    {
     ...
     * @return bool
     */
    public function mageCustomerLogin($arguments = array())
    {
        // Abort if the input is invalid
        if (empty($arguments) || empty($arguments['customer']['email'])) {
            return false;
    ...
    public function mageCustomerSaveAfter($arguments = array())
    {
        ...
        if (!isset($arguments['customer'])) return false;
        if (!isset($arguments['customer']['email'])) return false;
     ...
     * @return bool
     */
    public function mageCheckoutOnepageControllerSuccessAction($arguments = array())
    {
        // Fetch the arguments
        ...
        foreach ($products as $product) {
            ...
            $rt = MageBridgeConnectorProduct::getInstance()->runOnPurchase($product['sku'], $productQty, $user, $orderState, $arguments);
        }
     ...
     * @return bool
     */
    public function mageNewsletterSubscriberChangeAfter($arguments = array())
    {
        // Fetch the arguments
     ...
     * @return bool
     */
    public function mageSalesOrderSaveAfter($arguments = array())
    {
        // Fetch the arguments
        $products = $arguments['order']['products'];
        $customer = $arguments['order']['customer'];
        ...
        foreach ($products as $product) {
            $productQty = (isset($product['qty'])) ? $product['qty'] : 1;
            if ($productQty < 1) $productQty = 1; 
            $rt = MageBridgeConnectorProduct::getInstance()->runOnPurchase($product['sku'], $productQty, $user, $state, $arguments);
        }
