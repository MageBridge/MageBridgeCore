...
class MageBridgeTemplateHelper
{
    ...
    static public function removeMagentoScripts()
    {
        ...
        foreach ($headers['custom'] as $index => $header) {
            ...
            } else if (strstr($header, 'protoaculous')) {
                unset($headers['custom'][$index]);
            } 
        }
    ...
    {
        if (JRequest::getCmd('option') != 'com_magebridge') return true;
    ...
    static public function isHomePage()
    {
        ...
        if (JRequest::getCmd('option') == 'com_magebridge' && empty($request)) {
            return true;
        }
        return false;
    }
    ...
    {
        if (empty($request) && JRequest::getCmd('option') != 'com_magebridge') {
            return false;
        }
    ...
    static public function addMagentoStylesheet($file = null, $theme = 'default', $interface = 'default', $attribs = array())
    {
        ...
        if (!preg_match('/^(http|https):\/\//', $file)) $file = MageBridge::getBridge()->getMagentoUrl().'skin/frontend/'.$interface.'/'.$theme.'/css/'.$file;
		$document = JFactory::getDocument();
    ...
    static public function getPath($type, $file)
    {
        ...
        } else if (file_exists(JPATH_SITE.'/templates/'.$template.'/html/com_magebridge/'.$type.'/'.$file)) {
            $path = 'templates/'.$template.'/html/com_magebridge/'.$type.'/'.$file;

        } else if ($file == 'default.css' && file_exists(JPATH_SITE.'/media/com_magebridge/'.$type.'/default.'.$template.'.css')) {
            $path = 'media/com_magebridge/'.$type.'/'.'default.'.$template.'.css';

        } else if (file_exists(JPATH_SITE.'/media/com_magebridge/'.$type.'/'.$file)) {
        ...
        $root = JURI::root();
        if(JURI::getInstance()->isSSL() == true) $root = preg_replace('/^http:\/\//', 'https://', $root);
        if(JURI::getInstance()->isSSL() == false) $root = preg_replace('/^https:\/\//', 'http://', $root);
        if(!empty($path)) return $root.$path;
        return null;
    }
    ...
    {
        switch($variable) {
            case 'jquery': return 'jquery/jquery-1.8.1.min.js';
        }
    }
